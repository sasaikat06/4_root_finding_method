...
# Define the function for xe^x - 1 = 0 (rearranged from xe^x = 1)
def f(x):
    return x * np.exp(x) - 1

# Bisection method implementation
def bisection_method(f, a, b, tol=1e-6, max_iterations=11):  # Updated to 11 iterations
    if f(a) * f(b) >= 0:
...
# Print the result after 11 iterations
print(f"After 11 iterations, the root is approximately {root:.6f}")
...
# Plot the function
plt.plot(x_values, y_values, 'b-', label=r'$f(x) = xe^x - 1$')
...
# Set y-axis limits to zoom out
plt.ylim([-1, 2])  # Wider range to zoom out on the y-axis

# Labels and title
plt.title('Bisection Method for $xe^x - 1$ (Zoomed Out)')
plt.xlabel('x')
plt.ylabel('f(x)')
plt.grid(True)
