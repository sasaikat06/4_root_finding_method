...
# Implementing the Regula Falsi method with a limit of 4 iterations
def regula_falsi(f, a, b, max_iter=4):
    if f(a) * f(b) >= 0:
...
a = -3  # Guess from visual inspection
b = 3   # Guess from visual inspection

# Call the Regula Falsi method (limited to 4 iterations)
root, iteration_data = regula_falsi(f, a, b)

# Print the result after 4 iterations
print(f"Approximate root after 4 iterations: {root:.6f}")
...
plt.plot(x_values, y_values, 'b-', label=r'$f(x) = x^3 - 5x + 3$')
...
plt.plot(final_x, f(final_x), 'ro', markersize=10, label=f'Final Result: {final_x:.2f}')

# Write the final result on the graph
plt.text(final_x, f(final_x), f'  Root ≈ {final_x:.2f}', color='red', fontsize=12, verticalalignment='bottom')

# Set y-axis limits to zoom in around the root
plt.ylim([-10, 10])  # Adjust this range to zoom in around the y=0 region

# Labels and title
plt.title('Regula Falsi Method for $x^3 - 5x + 3 = 0$ (4 Iterations)')
plt.xlabel('x')
plt.ylabel('f(x)')
plt.grid(True)
